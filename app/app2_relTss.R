...
server <- function(input, output) {
  ...
  output$combinedPlot <- renderPlot({
    ...
    # Density plot
    density_plot <- ggplot() +
      geom_density(data = filtered_data()$data1, aes(x = exon_rel_tss, color = "Set 1"), fill = "blue", alpha = 0.1) +
      geom_density(data = filtered_data()$data2, aes(x = exon_rel_tss, color = "Set 2"), fill = "red", alpha = 0.1) +
      theme_minimal() +
    ...
    box_plot <- ggplot() +
      geom_boxplot(data = filtered_data()$data1, aes(x = exon_rel_tss, y = "Set 1"), fill = "blue", alpha = 0.1) +
      geom_jitter(data = filtered_data()$data1, aes(x = exon_rel_tss, y = "Set 1"), color = "blue", alpha = 0.1, height = 0.15) +
      geom_boxplot(data = filtered_data()$data2, aes(x = exon_rel_tss, y = "Set 2"), fill = "red", alpha = 0.1) +
      geom_jitter(data = filtered_data()$data2, aes(x = exon_rel_tss, y = "Set 2"), color = "red", alpha = 0.1, height = 0.15) +
      theme_minimal() +
